...
<html>
  <head>
	...
	<script type="text/javascript" src="${ctx}/dwr/interface/courseAction.js"></script>
 ...
 <c:if test="${session.user.position=='2' }">
	 <table style="background:#efefef; border-collapse:collapse ;"   width="100%" height="80" cellspacing="5" cellpadding="5"> 
	 	<tr>
		 	...
		 	<td width="15%"><input class="inputstyle2" type="text" id="class" /> </td>
	...
	<!--    <td width="15%" align="right"><label for="email">结束时间:</label></td>
 		...
 		<c:if test="${session.user.position=='3' }">
		  ...
		  <Label  id="selectMessage" class="btn btn-default" style="margin-right:20px;"  > 
		  请先填写选课信息！
		  </Label>
		</div>
</form> 
  ...
  <div id="dataList" style="display:none;" > 
    <table id="dg"  class="easyui-datagrid"  width="100%" 
        ...
        <thead>
            <tr>
                ...
                <th data-options="field:'status'"  width="15%">状态</th>
                 ...
                 <c:if test="${session.user.position=='3' }">
                	<th data-options="field:'_operate' ,formatter:formatOper1" width="15%">操作</th>
               </c:if>
            </tr>
        </thead>
    </table>  
     ...
     <div id="dlg" class="easyui-dialog"  style="padding:10px 30px;display:none"
			...
			<s:form name="fm" id="fm" action="" method="post">
				...
				<table border="0">
					<tr>
						<td>学号：</td>
						<td> <input id="studentid" name="course.studentId" class="easyui-textbox" readonly="readonly" style="height: 30px; width: 200px; " value="${session.user.userName}" /></td>
					</tr>
					<tr>
						<td>班级：</td>
						<td><input id="classId" name="course.classId" class="easyui-textbox" required="true" style="height: 30px; width: 200px; "/></td>
					</tr>
					<tr>
						<td>姓名：</td>
						<td><input id="name" name="course.name" class="easyui-textbox" required="true" style="height: 30px; width: 200px;  "/></td>
					</tr>
								
					<tr>
						<td>手机号：</td>
						<td><input id="tel" name="course.telno" class="easyui-textbox"  style="height: 30px; width: 200px;"/></td>
					</tr>
					<tr>
						<td>学年：</td>
						<td><input id="syear"  name="course.syear" class="easyui-textbox"  data-options="required:true,validType:'number'"  style="height: 30px; width: 200px;"/>(如:2015)</td>
					</tr>											
				</table>
    ...
    <div id="dlg-buttons">
        <a href="javascript:saveSelectCourse()" class="easyui-linkbutton c6" iconCls="icon-ok" onclick="javascript:void(0)" style="width:90px">确认</a>
